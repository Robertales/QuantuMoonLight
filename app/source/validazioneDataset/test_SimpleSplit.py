...
class TestSimpleSplit(unittest.TestCase):
    ...
    def test_simpleSplit(self):
        filename = pathlib.Path(__file__).cwd() / 'testingFiles' / 'bupa.csv'
        ...
        self.assertTrue(exists(pathlib.Path(__file__).cwd() / "Data_testing.csv"))
        self.assertTrue(exists(pathlib.Path(__file__).cwd() / "Data_training.csv"))
