...
def addClassPCAtraining(filename: Path, filenameOut: Path, numColsFE=2):
    ...
    with open(dataPath / "myPCAclass_Training.csv", "r") as input, open(
        dataPath / "IdPCADatasetTrain.csv", "w"
    ) as output:
    ...
    with open(dataPath / "IdPCADatasetTrain.csv") as input, open(
...
def addClassPCAtesting(filename: Path, filenameOut: Path, numColsFE=2):
    ...
    with open(dataPath / "myPCAclass_Testing.csv", "r") as input, open(
        dataPath / "IdPCADataset.csv", "w"
    ) as output:
        reader = csv.reader(input, delimiter=",")
        writer = csv.writer(output, delimiter=",")
    ...
    with open(dataPath / "IdPCADataset.csv") as input, open(
        filenameOut, "w", newline=""
    ) as output:
        writer = csv.writer(output)
        for row in csv.reader(input):
